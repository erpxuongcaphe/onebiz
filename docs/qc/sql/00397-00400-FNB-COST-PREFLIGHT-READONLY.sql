...
from (values
  ...
  ('public.inventory_checks'), ('public.stock_transfers'), ('public.stock_transfer_items'),
  ('public.supplier_returns'), ('public.disposal_exports'), ('public.internal_exports')
) as required(name);
...
where t.tgrelid = to_regclass('public.stock_movements')
  ...
  and t.tgname in ('capture_fnb_return_bom_cost_00396',
    ...
    'capture_fnb_purchase_revert_cost_00399', 'capture_fnb_inventory_cost_event_00400');
...
where p.oid in (
  ...
  to_regprocedure('public.complete_stock_transfer_atomic(uuid,uuid,uuid)')
);
commit;
