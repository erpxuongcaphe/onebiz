-- ============================================================
-- 00230 — GẤP: vá hồi quy do 00229 gây ra (thu tiền được nhiều lần)
-- ============================================================
-- PHÁT HIỆN bằng test đạn thật ngay sau khi áp 00229: bấm thanh toán lần
-- thứ hai trên cùng một đơn bếp KHÔNG bị chặn, đẻ thêm hoá đơn + phiếu thu
-- + trừ nguyên liệu lần nữa.
--
-- GỐC: guard nhận biết "đã thu tiền" bằng `status = 'completed'`. Nó chạy
-- được suốt thời gian qua là nhờ TÁC DỤNG PHỤ — thanh toán ép trạng thái
-- thành 'completed'. 00229 bỏ cái ép đó (để món ở lại màn bếp cho bếp làm
-- tiếp) nên guard mất chỗ dựa.
--
-- BÀI HỌC: khi tách hai khái niệm từng dính nhau (ở đây: "đã thu tiền" vs
-- "bếp đã xong"), phải rà lại MỌI chỗ đang dựa vào cái cũ để suy ra cái mới.
--
-- SỬA: bắt đúng dấu hiệu — CÓ invoice_id nghĩa là đã thu tiền, bất kể bếp
-- đang ở bước nào. Giữ luôn kiểm 'completed' làm lớp thứ hai.
--
-- CHƯA GÂY HẠI: prod chưa có hoá đơn F&B thật nào; lỗi chỉ tồn tại trong
-- khoảng giữa 00229 và file này. Không đụng dữ liệu.
-- ============================================================
...
begin
  ...
  -- 00230 SỬA HỒI QUY DO CHÍNH 00229 GÂY RA:
  -- Guard cũ nhận biết 'đã thu tiền' bằng status='completed' — chỉ đúng khi
  -- thanh toán còn ép trạng thái đó. 00229 bỏ ép (để món ở lại màn bếp) nên
  -- guard mất chỗ dựa: bấm thanh toán bao nhiêu lần cũng lọt, đẻ bấy nhiêu
  -- hoá đơn. Bắt đúng dấu hiệu: CÓ HOÁ ĐƠN nghĩa là đã thu tiền, bất kể bếp
  -- đang ở bước nào.
...
$$;
-- ============================================================
-- VERIFY — cả 3 phải true
-- ============================================================
-- select
--   pg_get_functiondef('public.fnb_complete_payment_atomic'::regproc) like '%v_order.invoice_id is not null%' as guard_theo_hoa_don,
--   pg_get_functiondef('public.fnb_complete_payment_atomic'::regproc) like '%for update%'                     as co_khoa_dong,
