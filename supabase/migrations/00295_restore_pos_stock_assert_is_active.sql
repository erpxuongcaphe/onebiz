-- 00295 — Khôi phục kiểm tra tồn POS sau khi định nghĩa cũ xuất hiện lại
...
-- Vá: chép nguyên hàm 00203, sửa 3 chỗ p.status → p.is_active (đánh dấu [00295]).
...
begin
  ...
  for v_item in
    ...
    select
      ...
      p.is_active, -- [00295] cột trạng thái kinh doanh thực tế
    ...
    group by p.id, p.name, p.is_active, p.has_bom -- [00295]
  loop
    ...
    -- [00295] chỉ chặn khi CHẮC CHẮN ngừng kinh doanh (is_active = false).
...
do $$
begin
  raise notice '00295 OK: assert_pos_stock_available dùng is_active — POS bán lại được.';
end $$;

-- Read-only verification. This only inspects the installed function definition.
select
  to_regprocedure('public.assert_pos_stock_available(uuid,uuid,jsonb,boolean)') is not null
    as pos_stock_assert_ok,
  position(
    'p.status' in pg_get_functiondef(
      to_regprocedure('public.assert_pos_stock_available(uuid,uuid,jsonb,boolean)')
    )
  ) = 0 as legacy_product_status_removed,
  position(
    'p.is_active' in pg_get_functiondef(
      to_regprocedure('public.assert_pos_stock_available(uuid,uuid,jsonb,boolean)')
    )
  ) > 0 as product_is_active_ok;