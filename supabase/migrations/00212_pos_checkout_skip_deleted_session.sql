-- 00212 — FIX: thanh toán bị chặn "Invoice ... is still draft" khi session của
-- máy còn dính NHÁP ĐÃ XÓA MỀM (nhân viên gặp 20/07 ~14:1x, F5 mới hết).
-- Gốc: idempotency check theo client_session_id trong pos_complete_checkout_atomic
-- KHÔNG lọc deleted_at → thấy nháp đã xóa vẫn coi là "còn draft" → raise.
-- Vá: chép NGUYÊN bản sống v6 (00208), chỉ thêm "and deleted_at is null" vào
-- 2 chỗ tra session (đánh dấu [00212]). Kèm code fix efa9dc3 (xóa mềm nhả
-- session) thì trạng thái này không tái sinh — migration này là lớp chống lại.
...
begin
  ...
  if v_session_id is not null then
    ...
    where tenant_id = p_tenant_id and client_session_id = v_session_id
      and deleted_at is null -- [00212] bỏ qua nháp đã xóa mềm
...
comment on function public.pos_complete_checkout_atomic is
  'Atomic retail POS checkout v7 (00212): v6 + idempotency bỏ qua nháp xóa mềm.';

notify pgrst, 'reload schema';

do $$
begin
  raise notice '00212 OK: checkout bỏ qua nháp đã xóa mềm khi tra session.';
end $$;
