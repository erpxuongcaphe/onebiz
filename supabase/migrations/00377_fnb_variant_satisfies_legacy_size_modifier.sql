-- 00377: a valid FnB product variant is the only Size source of truth.
-- Keep the original 00303 implementation body and all newer wrappers. The
-- only behavior change is that a validated variant satisfies the inherited
-- legacy group named Size. Other required modifiers are still mandatory.
-- No Retail function or business row is changed.

begin;

-- Preserve an executable private copy for an exact, data-free rollback. The
-- active implementation is replaced in place so wrapper dependencies keep
-- pointing at the same function OID.
do $guard$
declare
  ...
  v_backup regprocedure := to_regprocedure(
...
begin
  if v_current is null
    ...
    raise exception using errcode = 'P0001', message = 'FNB_00377_PREREQUISITE_MISSING';
  end if;
  v_definition := pg_get_functiondef(v_current);
  if position('PRODUCT_VARIANT_NOT_AVAILABLE' in v_definition) = 0
     or position('REQUIRED_MODIFIER_MISSING' in v_definition) = 0 then
    raise exception using errcode = 'P0001', message = 'FNB_00377_PREREQUISITE_CHANGED';
  end if;
  if v_backup is null then
    ...
    if position('_fnb_send_to_kitchen_impl_before_00377' in v_definition) = 0 then
      raise exception using errcode = 'P0001', message = 'FNB_00377_BACKUP_REWRITE_FAILED';
    end if;
...
alter function public._fnb_send_to_kitchen_impl_before_00377(
  uuid,uuid,text,text,text,jsonb,text,numeric,numeric,uuid,text,uuid
) owner to postgres;
revoke all on function public._fnb_send_to_kitchen_impl_00303(
  uuid,uuid,text,text,text,jsonb,text,numeric,numeric,uuid,text,uuid
) from public, anon, authenticated, service_role;
revoke all on function public._fnb_send_to_kitchen_impl_before_00377(
  uuid,uuid,text,text,text,jsonb,text,numeric,numeric,uuid,text,uuid
) from public, anon, authenticated, service_role;
...
declare
  ...
  v_public_definition text := pg_get_functiondef(
...
begin
  if to_regprocedure(
     ...
     or position('00377_VARIANT_SATISFIES_LEGACY_SIZE' in v_definition) = 0
     or position('00377_REJECT_DUPLICATE_LEGACY_SIZE' in v_definition) = 0
     or position('REQUIRED_MODIFIER_MISSING' in v_definition) = 0
     or position('_fnb_send_to_kitchen_impl_00353' in v_public_definition) = 0 then
    raise exception using errcode = 'P0001', message = 'FNB_00377_INSTALL_INCOMPLETE';
  end if;
end;
$verify$;
