-- ============================================================
-- Migration 00214: HOÀN NHẬP — cảnh báo tồn âm nhưng VẪN cho hủy
-- ============================================================
--
-- CEO 21/07/2026: khi hoàn nhập phiếu đã nhận mà hàng đã bán/dùng bớt
-- (tồn < đã nhận), TRƯỚC chặn cứng (raise). Nay: UI hiện cảnh báo đỏ,
-- nếu quản lý xác nhận → truyền p_allow_negative=true → RPC bỏ qua chặn,
-- hoàn nhập tiếp (tồn có thể xuống âm, chấp nhận, kiểm kho sau).
--
-- Thân hàm COPY NGUYÊN VĂN bản sống 00149, chỉ:
--   (1) thêm param p_allow_negative boolean default false
--   (2) bọc điều kiện chặn tồn âm bằng "not p_allow_negative"
-- Mặc định false → hành vi CŨ giữ nguyên cho mọi caller chưa truyền cờ.
--
-- ⚠️ Đổi CHỮ KÝ (thêm param) → phải DROP hàm 2-tham-số cũ trước khi tạo
-- (nếu không sẽ tạo overload → gọi mơ hồ). Grant lại sau.
-- ============================================================

-- Bỏ hàm cũ (2 tham số) để thay bằng bản 3 tham số.
drop function if exists public.revert_received_purchase_order_atomic(uuid, uuid);

create or replace function public.revert_received_purchase_order_atomic(
  p_order_id uuid,
  p_user_id uuid,
  p_allow_negative boolean default false  -- [00214] cho phép hoàn nhập dù tồn âm (CEO duyệt: cảnh báo nhưng vẫn cho hủy)
) returns jsonb
language plpgsql
security definer
...
begin
  ...
  loop
    ...
    -- [00214] chỉ chặn khi KHÔNG cho phép âm; cho phép → hoàn nhập tiếp (tồn có thể âm, quản lý tự chịu)
    if not p_allow_negative and v_current_stock - v_qty_to_revert < 0 then
      raise exception
...
grant execute on function public.revert_received_purchase_order_atomic(uuid, uuid, boolean) to authenticated;

-- ============================================================
-- VERIFY (read-only)
-- ============================================================
select proname,
       pronargs as so_tham_so,
       prosrc like '%[00214]%' as da_co_co
  from pg_proc
 where proname = 'revert_received_purchase_order_atomic';
-- Kỳ vọng: 1 dòng, so_tham_so=3, da_co_co=true.
