-- 00312: Read-only workspace for the purchase-order list.
-- This migration creates one STABLE function. It does not update business data.

create or replace function public.get_purchase_order_list_workspace(
...
begin
  if v_actor is null then
    raise exception using errcode = '42501', message = 'PURCHASE_ORDER_LIST_AUTH_REQUIRED';
  end if;

  v_tenant := public.get_user_tenant_id();
  if v_tenant is null then
    raise exception using errcode = '42501', message = 'PURCHASE_ORDER_LIST_TENANT_UNKNOWN';
  end if;
  ...
  if not exists (
    ...
    raise exception using errcode = '42501', message = 'PURCHASE_ORDER_LIST_PROFILE_INACTIVE';
  end if;

  if not public.user_has_permission(v_actor, 'inventory.view') then
    raise exception using errcode = '42501', message = 'PURCHASE_ORDER_LIST_PERMISSION_DENIED';
  end if;
  ...
  if p_branch_id is not null then
    if not exists (
      ...
      raise exception using errcode = '42501', message = 'PURCHASE_ORDER_LIST_BRANCH_NOT_IN_TENANT';
    end if;
    if not v_can_all and not public.user_has_branch_access(v_actor, p_branch_id) then
      raise exception using errcode = '42501', message = 'PURCHASE_ORDER_LIST_BRANCH_DENIED';
    end if;
  end if;

  if p_amount_min is not null and p_amount_max is not null and p_amount_min > p_amount_max then
    raise exception using errcode = '22023', message = 'PURCHASE_ORDER_LIST_AMOUNT_RANGE_INVALID';
  end if;
  ...
  with scoped as (
    ...
    left join public.profiles p
      on p.id = po.created_by and p.tenant_id = v_tenant
    left join public.branches b
      on b.id = po.branch_id and b.tenant_id = v_tenant
    left join public.suppliers s
      on s.id = po.supplier_id and s.tenant_id = v_tenant
    where po.tenant_id = v_tenant
      and (
        (p_branch_id is not null and po.branch_id = p_branch_id)
    ...
    where (
      ...
      and (
        ...
        or (p_status = 'pending' and s.status in ('draft', 'ordered'))
    ...
    select
      ...
      count(*) filter (where status in ('draft', 'ordered', 'partial'))::bigint as outstanding_count,
      coalesce(sum(total) filter (where status in ('draft', 'ordered', 'partial')), 0)::numeric as outstanding_value,
  ...
  select jsonb_build_object(
    ...
    'summary', jsonb_build_object(
      'outstandingCount', (select outstanding_count from filtered_summary),
      'outstandingValue', (select outstanding_value from filtered_summary),
      'completedCount', (select completed_count from filtered_summary),
...
comment on function public.get_purchase_order_list_workspace(
  integer, integer, text, text, text, timestamptz, timestamptz, numeric, numeric, uuid
) is 'Read-only purchase-order list, metrics, filters and branch scope.';

revoke all on function public.get_purchase_order_list_workspace(
  integer, integer, text, text, text, timestamptz, timestamptz, numeric, numeric, uuid
) from public, anon;

grant execute on function public.get_purchase_order_list_workspace(
  integer, integer, text, text, text, timestamptz, timestamptz, numeric, numeric, uuid
) to authenticated;
...
select to_regprocedure(
  'public.get_purchase_order_list_workspace(integer,integer,text,text,text,timestamp with time zone,timestamp with time zone,numeric,numeric,uuid)'
) is not null as purchase_order_workspace_rpc_ok;
