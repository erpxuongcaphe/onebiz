-- 00381: Allow a retail sales return to create customer credit after invoice debt is cleared.
--
-- Applying this migration changes only the stored procedure definition. It does
-- not create returns, alter cash, change stock, or rewrite existing customer debt.

begin;

-- Keep an exact, callable copy of the previously deployed implementation so
-- rollback is definition-only as well. This copy is never granted to clients.
do $$
declare
  v_definition text;
begin
  ...
  if v_definition is null then
    raise exception 'SALES_RETURN_IMPLEMENTATION_NOT_FOUND';
  end if;
...
revoke all on function public._create_sales_return_auth_impl_00244_pre_00381(
...
declare
  ...
  v_invoice_debt_reduction numeric := 0;
...
begin
  ...
  if p_shift_id is not null and not exists (
    select 1 from public.shifts s
     where s.id = p_shift_id and s.tenant_id = v_tenant_id
       and s.branch_id = v_invoice.branch_id and s.cashier_id = v_actor and s.status = 'open'
  ) then
  ...
  loop
    ...
    select ii.id, ii.product_id, ii.product_name, ii.unit, ii.quantity,
           ii.total, coalesce(ii.returned_qty, 0) as returned_qty, ii.variant_id
      into v_line
      from public.invoice_items ii
     where ii.id = v_invoice_item_id and ii.invoice_id = v_invoice.id
     for update;
    if not found then
  ...
  -- First settle no more than the original invoice debt. The excess is a real
  -- customer balance, represented as a negative debt adjustment for later use.
  v_debt_credit := v_total - v_refund;
  v_invoice_debt_reduction := least(v_debt_credit, greatest(coalesce(v_invoice.debt, 0), 0));
  v_customer_credit := v_debt_credit - v_invoice_debt_reduction;
  if v_customer_credit > 0 and v_invoice.customer_id is null then
    raise exception 'CUSTOMER_REQUIRED_FOR_CREDIT_BALANCE' using errcode = 'P0001';
  end if;
  ...
  insert into public.sales_returns (
    ...
    v_invoice.customer_id, v_invoice.customer_name, 'completed', v_total, v_refund,
    nullif(trim(p_reason), ''), nullif(trim(p_note), ''), v_actor
  ...
  loop
    ...
    select p.id, coalesce(p.has_bom, false) as has_bom, p.inventory_role
      into v_product
      from public.products p
     where p.id = v_line.product_id and p.tenant_id = v_tenant_id
       and coalesce(p.is_active, true);
    if not found then
    ...
    if v_variant_id is null then
      select koi.variant_id into v_variant_id
        from public.kitchen_orders ko
        join public.kitchen_order_items koi on koi.kitchen_order_id = ko.id
       where ko.invoice_id = v_invoice.id and koi.product_id = v_line.product_id
         and v_line.product_name = case
           when nullif(koi.variant_label, '') is not null then koi.product_name || ' (' || koi.variant_label || ')'
    ...
    if v_product.inventory_role = 'fnb_menu_item' or v_product.has_bom then
      ...
      if not v_bom_found and v_product.inventory_role = 'fnb_menu_item' then
        v_warnings := v_warnings || jsonb_build_object(
          'code', 'MENU_RECIPE_NOT_FOUND', 'invoice_item_id', v_invoice_item_id,
        ...
        insert into public.stock_movements (
          ...
          v_return_code || ' - Tra hang (SKU chua co BOM) - ' || v_line.product_name, v_actor
        );
        perform public.increment_product_stock(v_line.product_id, v_qty);
        perform public.upsert_branch_stock(v_tenant_id, v_invoice.branch_id, v_line.product_id, v_qty);
      end if;
    else
      insert into public.stock_movements (
        tenant_id, branch_id, product_id, type, quantity,
        ...
        v_tenant_id, v_invoice.branch_id, v_line.product_id, 'in', v_qty,
        'sales_return', v_return_id, v_return_code || ' - Tra hang - ' || v_line.product_name, v_actor
      );
      perform public.increment_product_stock(v_line.product_id, v_qty);
      perform public.upsert_branch_stock(v_tenant_id, v_invoice.branch_id, v_line.product_id, v_qty);
    end if;
    ...
    update public.invoice_items
     ...
     where id = v_invoice_item_id and invoice_id = v_invoice.id
  ...
  if v_refund > 0 then
    ...
    insert into public.cash_transactions (
      ...
      v_refund, v_invoice.customer_name, v_payment_method, 'sales_return', v_return_id,
      'Hoan tien ' || v_return_code || ' (HD goc: ' || v_invoice.code || ')', v_actor, p_shift_id
    );
  end if;

  if v_invoice_debt_reduction > 0 then
    update public.invoices
       set debt = debt - v_invoice_debt_reduction
     where id = v_invoice.id and tenant_id = v_tenant_id and debt >= v_invoice_debt_reduction;
    get diagnostics v_rows = row_count;
    if v_rows <> 1 then
  ...
  if v_customer_credit > 0 then
    -- Serialize the customer ledger recomputation with other returns and credit uses.
    perform 1 from public.customers c
     where c.id = v_invoice.customer_id and c.tenant_id = v_tenant_id
     for update;
    if not found then
      raise exception 'CUSTOMER_NOT_FOUND_FOR_CREDIT_BALANCE' using errcode = 'P0001';
    end if;
    insert into public.customer_debt_adjustments (
      tenant_id, customer_id, invoice_id, amount, reason, idempotency_key, created_by
    ) values (
      v_tenant_id, v_invoice.customer_id, v_invoice.id, -v_customer_credit,
      'So du co tu tra hang ' || v_return_code || ' (HD goc: ' || v_invoice.code || ')',
      'sales-return-credit:' || v_return_id::text, v_actor
    );
  end if;
  ...
  insert into public.audit_log (
    ...
    v_tenant_id, v_actor, 'create', 'sales_return', v_return_id,
    jsonb_build_object(
      'code', v_return_code, 'invoice_id', v_invoice.id, 'invoice_code', v_invoice.code,
      'branch_id', v_invoice.branch_id, 'total', v_total, 'refunded', v_refund,
      'debt_credit', v_debt_credit, 'invoice_debt_reduction', v_invoice_debt_reduction,
      'customer_credit', v_customer_credit, 'warnings', v_warnings, 'atomic', true
    )
  );

  return jsonb_build_object(
    'return_id', v_return_id, 'code', v_return_code, 'total', v_total,
    'refunded', v_refund, 'debt_credit', v_debt_credit,
    'invoice_debt_reduction', v_invoice_debt_reduction,
    'customer_credit', v_customer_credit, 'warnings', v_warnings
  );
end;
$$;
...
select
  to_regprocedure('public._create_sales_return_auth_impl_00244(uuid,jsonb,numeric,text,text,text,uuid)') is not null as retail_return_credit_impl_ok,
  pg_get_functiondef('public._create_sales_return_auth_impl_00244(uuid,jsonb,numeric,text,text,text,uuid)'::regprocedure) like '%customer_debt_adjustments%' as customer_credit_ledger_ok,
  pg_get_functiondef('public._create_sales_return_auth_impl_00244(uuid,jsonb,numeric,text,text,text,uuid)'::regprocedure) not like '%DEBT_CREDIT_EXCEEDS_INVOICE_DEBT%' as no_invoice_debt_ceiling;
