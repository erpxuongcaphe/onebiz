...
export function FnbCart({
  ...
  return (
    <div className={cn(
      ...
      <div className="border-t bg-gray-50 p-4 md:p-3 shrink-0 space-y-2.5 md:space-y-2">
        {/* Subtotal */}
...
function DiscountRow({
  ...
  return (
    <div className="flex items-center gap-1.5">
      <span className="text-xs text-gray-500 shrink-0">Giảm giá</span>
      <Input
        type="text"
        inputMode="numeric"
        value={localValue}
      ...
      {line.toppings.length > 0 && (
        <div className="mt-1 space-y-0.5">
          {line.toppings.map((t, i) => (
            <p key={i} className="text-[11px] text-gray-400 pl-2">
              + {t.name} x{t.quantity}{" "}
              <span className="text-gray-500">{formatCurrency(t.price)}</span>
            </p>
          ))}
        </div>
      )}
