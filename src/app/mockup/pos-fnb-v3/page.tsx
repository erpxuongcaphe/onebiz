...
/**
 * MOCKUP v3 — POS F&B refined sau feedback CEO
 * Route: /mockup/pos-fnb-v3
 *
 * Sửa 6 điểm anh phản hồi từ v2:
 *  1. Tinh tế hơn: typography & spacing chuẩn, chip cùng kích thước, layout cân.
 *  2. Bớt phải chọn: defaults rõ ràng + nút "Thêm + giá" sticky dưới → add nhanh.
 *  3. Topping nhiều không bị dài: chip cuộn ngang, tap để +1, stepper khi đã chọn.
 *  4. Ô món gọn hơn: tile NGANG (icon + tên + giá) thay tile vuông to.
 *  5. Sơ đồ bàn: toggle Thực đơn / Sơ đồ bàn + tab khu vực + lưới bàn trạng thái.
 *  6. Cài đặt POS: nút bánh răng → panel cài đặt.
 *
...
// Thực đơn 8 topping → demo trường hợp "nhiều topping" CEO lo
...
const PRODUCTS: ProductV3[] = [
  { id: "p1", name: "Cà phê sữa đá", price: 29000, catId: "cafe", fav: true, sizes: SIZE_ML, sugar: true, ice: true },
  ...
  { id: "p4", name: "Cà phê muối", price: 39000, catId: "cafe", sizes: SIZE_ML, sugar: true, ice: true },
  { id: "p5", name: "Cappuccino", price: 45000, catId: "cafe", sizes: SIZE_ML },
  { id: "p6", name: "Latte", price: 45000, catId: "cafe", sizes: SIZE_ML },
  { id: "p7", name: "Cold Brew", price: 49000, catId: "cafe" },
  { id: "p8", name: "Espresso", price: 35000, catId: "cafe" },
  { id: "p9", name: "Trà sữa trân châu đường đen", price: 45000, catId: "trasua", fav: true, sizes: SIZE_ML, sugar: true, ice: true, toppings: TOPPING_DRINK },
  { id: "p10", name: "Trà sữa truyền thống", price: 39000, catId: "trasua", sizes: SIZE_ML, sugar: true, ice: true, toppings: TOPPING_DRINK },
  { id: "p11", name: "Trà sữa matcha", price: 45000, catId: "trasua", sizes: SIZE_ML, sugar: true, ice: true, toppings: TOPPING_DRINK },
  { id: "p12", name: "Trà sữa khoai môn", price: 42000, catId: "trasua", sizes: SIZE_ML, sugar: true, ice: true, toppings: TOPPING_DRINK },
  { id: "p13", name: "Hồng trà sữa", price: 39000, catId: "trasua", sizes: SIZE_ML, sugar: true, ice: true, toppings: TOPPING_DRINK },
  { id: "p14", name: "Trà đào cam sả", price: 45000, catId: "tra", fav: true, sizes: SIZE_ML, sugar: true, ice: true, toppings: TOPPING_DRINK },
  { id: "p15", name: "Trà vải", price: 42000, catId: "tra", sizes: SIZE_ML, sugar: true, ice: true, toppings: TOPPING_DRINK },
  { id: "p16", name: "Trà tắc", price: 32000, catId: "tra", sugar: true, ice: true },
  { id: "p17", name: "Trà ổi hồng", price: 45000, catId: "tra", sizes: SIZE_ML, sugar: true, ice: true, toppings: TOPPING_DRINK },
  { id: "p18", name: "Matcha đá xay", price: 55000, catId: "daxay", sizes: SIZE_ML, sugar: true, toppings: TOPPING_DRINK },
  { id: "p19", name: "Socola đá xay", price: 55000, catId: "daxay", sizes: SIZE_ML, sugar: true },
  { id: "p20", name: "Cookie đá xay", price: 59000, catId: "daxay", sizes: SIZE_ML },
  { id: "p21", name: "Bánh tiramisu", price: 45000, catId: "banh" },
  { id: "p22", name: "Bánh mousse chanh dây", price: 42000, catId: "banh" },
  { id: "p23", name: "Croissant bơ", price: 32000, catId: "banh" },
  { id: "p24", name: "Bánh phô mai nướng", price: 39000, catId: "banh" },
  { id: "p25", name: "Cookie socola", price: 22000, catId: "banh" },
  { id: "p26", name: "Nước suối", price: 10000, catId: "chai" },
  { id: "p27", name: "Coca", price: 15000, catId: "chai" },
  { id: "p28", name: "7Up", price: 15000, catId: "chai" },
];

function hasOptions(p: ProductV3): boolean {
...
interface CartLineV3 { lineId: string; product: ProductV3; selection: Selection }

function defaultSelection(p: ProductV3): Selection {
  return {
    sizeId: p.sizes?.[0]?.id,
...
function unitPriceOf(p: ProductV3, s: Selection): number {
...
function selectionSummary(p: ProductV3, s: Selection): string {
  const parts: string[] = [];
...
export default function PosFnbV3Mockup() {
  ...
  const [lines, setLines] = useState<CartLineV3[]>([]);
  const [toast, setToast] = useState<string | null>(null);
  const [modifier, setModifier] = useState<{ product: ProductV3; lineId: string | null; sel: Selection } | null>(null);
  ...
  function tapProduct(p: ProductV3) {
    if (hasOptions(p)) {
  ...
  return (
    <div className="flex h-screen flex-col overflow-hidden bg-background text-foreground">
      ...
      <div className="shrink-0 bg-primary-fixed px-4 py-1.5 text-xs text-primary">
        ...
        <span className="font-semibold">MOCKUP v3 — refined.</span>{" "}
        <span className="opacity-80">
          Sửa: chip đồng cỡ · sticky “Thêm” không cần cuộn · topping chip cuộn ngang · ô món gọn · có Sơ đồ bàn · có Cài đặt.
        </span>
      </div>
      ...
      <div className="flex flex-1 min-h-0">
        {view === "menu" ? (
          <>
            {/* Sidebar danh mục — gọn hơn (180px) */}
            ...
            {/* Lưới món — tile NGANG compact */}
            <div className="flex-1 min-w-0 overflow-y-auto p-3">
              <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 xl:grid-cols-4">
                {products.map((p) => {
                  const c = CAT_BY[p.catId];
                  ...
                  return (
                    <button
                      ...
                      className="group relative flex h-[72px] items-center gap-2.5 overflow-hidden rounded-lg border border-outline-variant/20 bg-card px-2.5 text-left transition-all duration-150 press-scale-sm hover:ambient-shadow hover:border-outline-variant/40"
                    >
                      <span className="flex size-10 shrink-0 items-center justify-center rounded-lg text-white" style={{ backgroundColor: c?.color ?? "#999" }}>
                        <Icon name={c?.icon ?? "local_cafe"} size={18} />
                      </span>
                      ...
                      {p.fav && (
                        <Icon name="star" size={11} className="absolute right-1.5 top-1.5 text-status-warning" />
                      )}
                      {inCart > 0 && (
...
// ════════════════════════════════════════════════════════════════
// Bảng tuỳ chọn — REFINED v3
// • Header tinh gọn (title + close + giá hiện tại)
// • Chip đồng kích thước (h-9, min-width)
// • Topping = CHIP cuộn ngang/quấn, tap = +1, stepper khi đã chọn
// • Sticky footer "Thêm + giá" → không cần cuộn vẫn add được
// ════════════════════════════════════════════════════════════════
function ModifierSheet({
  state, onChange, onConfirm, onClose,
}: {
  state: { product: ProductV3; lineId: string | null; sel: Selection };
  onChange: (s: { product: ProductV3; lineId: string | null; sel: Selection }) => void;
  onConfirm: () => void;
  onClose: () => void;
}) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 sm:items-center" onClick={onClose}>
      ...
      >
        {/* Header refined — 1 dòng, không subtitle thừa */}
        ...
        {/* Sticky footer — 1 nút lớn, không cần cuộn vẫn thấy + add */}
        <div className="flex shrink-0 items-center gap-2 border-t border-outline-variant/15 px-4 py-3">
          ...
          <button
            onClick={onConfirm}
            className="flex h-11 flex-1 items-center justify-between gap-2 rounded-lg bg-primary px-4 text-sm font-bold text-on-primary ambient-shadow press-scale-sm"
          >
  ...
  return (
    <div>
      <div className="mb-1.5 flex items-center gap-2">
        ...
        {rule && (
          <span className="rounded bg-surface-container px-1.5 text-[9px] font-bold uppercase tracking-wider text-muted-foreground">
            {rule}
          </span>
        )}
      </div>
...
function CartLine({
  ...
  line: CartLineV3; onInc: () => void; onDec: () => void; onRemove: () => void; onEdit: () => void;
}) {
