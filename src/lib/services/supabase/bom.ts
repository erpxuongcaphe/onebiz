...
function mapBOMItem(row: Record<string, unknown>): BOMItem {
  ...
  return {
    ...
    inputQuantity: (row.input_quantity as number) ?? undefined,
    inputUnit: (row.input_unit as string) ?? undefined,
    conversionFactor: (row.conversion_factor as number) ?? undefined,
