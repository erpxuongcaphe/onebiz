...
/**
 * CEO 22/05/2026: Tính trước mã SP đầu tiên cho group_code + prefix.
 *
 * Áp dụng logic dedupe của RPC `next_group_code` (migration 00110):
 *   - Nếu group_code đã bắt đầu bằng "{prefix}-" thì KHÔNG concat lần nữa.
 *   - Vd: previewProductCode('NVL', 'CAFE')      → 'NVL-CAFE-001'
 *        previewProductCode('SKU', 'SKU-TEST')   → 'SKU-TEST-001' (dedupe)
 *        previewProductCode('NVL', 'NVL-BAO')    → 'NVL-BAO-001'  (dedupe)
 *
 * Dùng cho UI preview ngay tại form tạo nhóm — KHÔNG cần round-trip server.
 */
export function previewProductCodeFromGroup(
  prefix: "NVL" | "SKU",
  groupCode: string,
): string {
  const upper = groupCode.toUpperCase().trim();
  const prefixPattern = `${prefix.toUpperCase()}-`;
  const effective = upper.startsWith(prefixPattern)
    ? upper.slice(prefixPattern.length)
    : upper;
  return `${prefix}-${effective}-001`;
}
