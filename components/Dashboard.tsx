...
import { REVENUE_DATA, CATEGORY_DATA, MOCK_ACTIVITIES, formatCurrency } from '../constants';
...
const Dashboard: React.FC = () => {
  ...
  const revenueValue = overview ? formatCurrency(overview.revenue7d) : formatCurrency(45000000);
  const profitValue = overview ? formatCurrency(overview.profit7d) : formatCurrency(8500000);
  const cashValue = overview ? formatCurrency(overview.cash7d) : formatCurrency(280000000);
  const debtValue = overview ? formatCurrency(overview.debt) : formatCurrency(125000000);
  const series = overview?.series7d ?? REVENUE_DATA;
